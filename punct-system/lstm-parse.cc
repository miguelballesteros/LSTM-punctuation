	...
	static map<int,string> compute_puncts(unsigned sent_len, const vector<unsigned>& actions, const vector<string>& setOfActions, map<int,string>* pr = nullptr) {
	  map<int,string> puncts;
	  map<int,string> r;
	  map<int,string>& rels = (pr ? *pr : r);
	  for(unsigned i=0;i<sent_len;i++) { puncts[i]="empty"; rels[i]="ERROR"; }
	  ...
	  for (auto action: actions) { // loop over transitions for sentence
		...
		stacki.push_back(-100);
		puncts[cont]=actionString;        
		cont++;
		previousPunct=true;
	    }
	...
	vector<unsigned> log_prob_parser(ComputationGraph* hg,
	  ...
	  //cerr << endl;
		...
		if (IsActionForbidden(setOfActions[a], buffer.size(), stack.size(), stacki))
		  continue;
                if (!(last_action[0] =='G' && setOfActions[a][0]=='G'))
      ...
      else if (ac=='G') { //GENERATING PUNCTUATION
          stack.push_back(relation);
          stack_lstm.add_input(relation); 
          stacki.push_back(-100);
...
unsigned compute_correct(const map<int,string>& ref, const map<int,string>& hyp, unsigned len, map<string,int>& correctPerClass,map<string,int>& predictedPerClass, map<string,int>& goldPerClass, vector<string> actions) {
  unsigned res = 0;
...
/* for (auto a: actions) {
    ...
    if (ac=='G') {*/
         for (unsigned i = 0; i < len; ++i) {
            ...
            string prediction=hi->second;
            string gold=ri->second;


            goldPerClass[gold]=goldPerClass[gold]+1;
            predictedPerClass[prediction]=predictedPerClass[prediction]+1;
            if (ri->second == hi->second) correctPerClass[prediction]=correctPerClass[prediction]+1;
         }
     //}  
  //}
...
void output_conll(const vector<unsigned>& sentence, const vector<unsigned>& pos,
                  ...
                  const map<int,string>& hyp, const map<int,string>& rel_hyp) {
  for (unsigned i = 0; i < (sentence.size()); ++i) {
   //  cout<<intToWords(sentence[i])<<hyp.find(i)->second<<"\n";
       if (hyp.find(i)->second=="empty")
          cout<<intToWords.find(sentence.at(i))->second<<" ";
       else {
	  cout<<hyp.find(i)->second<<" "<<intToWords.find(sentence.at(i))->second<<" ";
       }
   }
   cout<<"\n";
}
...
int main(int argc, char** argv) {
  ...
  if (conf.count("train")) {
    ...
    while(!requested_stop) {
      ...
      if (logc % 25 == 1) { // report on dev set
        ...
        double trs = 0;
        double right = 0;
        double correct_puncts = 0;
        double total_puncts = 0;
        auto t_start = std::chrono::high_resolution_clock::now();

        map<string,int> correctPerClass;
        map<string,int> predictedPerClass;
        map<string,int> goldPerClass;
	   ...
	   double lp = 0;
           ...
           llh -= lp;
           trs += actions.size();
           map<int,string> ref = parser.compute_puncts(sentence.size(), actions, corpus.actions);
           map<int,string> hyp = parser.compute_puncts(sentence.size(), pred, corpus.actions);
           //output_conll(sentence, corpus.intToWords, ref, hyp);
           //correct_puncts += compute_correct(ref, hyp, sentence.size() - 1);
           correct_puncts += compute_correct(ref, hyp, sentence.size(), correctPerClass, predictedPerClass, goldPerClass, corpus.actions);
           total_puncts += sentence.size();

        }
        double avrecall=0.0;
        double avprecision=0.0;
        int numbclass=0;

        map<string,int>::iterator gt = goldPerClass.begin();
           for(map<string,int>::iterator it = correctPerClass.begin(); it != correctPerClass.end(); ++it) {
             avrecall+=(double) it->second/goldPerClass.at(it->first);
             gt++;
             numbclass++;
         }


         map<string,int>::iterator pt = predictedPerClass.begin();
         for(map<string,int>::iterator it = correctPerClass.begin(); it != correctPerClass.end(); ++it) {
           avprecision+=(double) it->second/predictedPerClass.at(it->first);
           pt++;
         }

         avrecall=avrecall/numbclass;
         avprecision=avprecision/numbclass;

         f1score=(2*avrecall*avprecision)/(avrecall+avprecision);
         


        auto t_end = std::chrono::high_resolution_clock::now();
        //cerr << "  **dev (iter=" << iter << " epoch=" << (tot_seen / corpus.nsentences) << ")\tllh=" << llh << " ppl: " << exp(llh / trs) << " err: " << (trs - right) / trs << " f1: " <<  f1score << "\t[" << dev_size << " sents in " << std::chrono::duration<double, std::milli>(t_end-t_start).count() << " ms]" << endl;
        cerr << "  **dev (iter=" << iter << " epoch=" << (tot_seen / corpus.nsentences) << ")\tllh=" << llh << " ppl: " << exp(llh / trs) << " err: " << (trs - right) / trs << " res: " << (correct_puncts / total_puncts) << "\t[" << dev_size << " sents in " << std::chrono::duration<double, std::milli>(t_end-t_start).count() << " ms]" << endl;
        /*if (f1score > best_f1) {
          best_f1 = f1score;*/
        if (correct_puncts > best_correct_heads) {
          best_correct_heads = correct_puncts;
          ofstream out(fname);
  ...
  if (true) { // do test evaluation
    ...
    for (unsigned sii = 0; sii < corpus_size; ++sii) {
      ...
      map<int,string> ref = parser.compute_puncts(sentence.size(), actions, corpus.actions, &rel_ref);
      map<int,string> hyp = parser.compute_puncts(sentence.size(), pred, corpus.actions, &rel_hyp);
      ...
      cerr<<"predicted per class"<<"\n";
      for(map<string,int>::iterator it = predictedPerClass.begin(); it != predictedPerClass.end(); ++it) {
         cerr << it->first << "-->"<< it->second << "\n";
      }  
      cerr<<"\n"; 

      cerr<<"Gold per class"<<"\n";
      for(map<string,int>::iterator it = goldPerClass.begin(); it != goldPerClass.end(); ++it) {
         cerr << it->first << "-->"<< it->second << "\n";
      }
     ...
     cerr<<"Recall per class"<<"\n";
     map<string,int>::iterator gt = goldPerClass.begin();
     for(map<string,int>::iterator it = correctPerClass.begin(); it != correctPerClass.end(); ++it) {
        cerr << it->first << "-->"<< (double) it->second/goldPerClass.at(it->first) << "\n";
        gt++;
     }

     cerr<<"PRECISION"<<"\n";

     cerr<<"PRECISION per class"<<"\n";
     map<string,int>::iterator pt = predictedPerClass.begin();
     for(map<string,int>::iterator it = correctPerClass.begin(); it != correctPerClass.end(); ++it) {
        cerr << it->first << "-->"<< (double) it->second/predictedPerClass.at(it->first) << "\n";
        pt++;
     }
      ...
      avrecall=avrecall/numbclass;
      avprecision=avprecision/numbclass;
      cerr<<"av.rec:"<<avrecall<<"\n";
      cerr<<"av.prec:"<<avprecision<<"\n";

      f1score=(2*avrecall*avprecision)/(avrecall+avprecision); 
      cerr<<"f1:"<<f1score<<"\n";

    auto t_end = std::chrono::high_resolution_clock::now();
    //cerr << "TEST llh=" << llh << " ppl: " << exp(llh / trs) << " err: " << (trs - right) / trs << " f1: " << f1score<< "\t[" << corpus_size << " sents in " << std::chrono::duration<double, std::milli>(t_end-t_start).count() << " ms]" << endl;
    cerr << "TEST llh=" << llh << " ppl: " << exp(llh / trs) << " err: " << (trs - right) / trs << " res: " << (correct_puncts / total_puncts) << "\t[" << corpus_size << " sents in " << std::chrono::duration<double, std::milli>(t_end-t_start).count() << " ms]" << endl;
